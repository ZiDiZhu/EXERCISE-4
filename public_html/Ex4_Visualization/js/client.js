...
function displayFour(resultSet,propOne){
  dataPoints =[];
  let finalHeight =0;

  //set background of parent ... for fun ..
    document.getElementById("parent-wrapper").style.background = "black";
    description.textContent = `greens = positive (after)moods, grays = negative. grouped by 7 days of the week (Monday>>Sunday)`;
    description.style.color = "yellow";

    let coloredEvents = {}

    let possibleEvents = resultSet[resultSet.length-1];
    let possibleColors = ['lime','grey','gray','lightgreen','lawngreen','dimgray','springgreen','gray','yellow','darkgray'];
  ...
  let yPos =offsetY;
    ...
    for(let i = 0; i<resultSet.length-1; i++){
      dataPoints.push(new myDataPoint(resultSet[i].dataId,
        ...
        resultSet[i].eID,
        //map to the EVENT ...
        coloredEvents[resultSet[i].after_mood],
        //last parameter is where should this go...
        document.getElementById("childOne"),
        //which css style///
        "pointFour"
